...
$checkStatistic = "SELECT * FROM navstevnost stat WHERE stat.id = 3";
$resultStatistic = $conn->query($checkStatistic);
if($rowStatistic = $resultStatistic->fetch_assoc()){
    $checkStatistic = $rowStatistic["count"] + 1;
    $statistic = "UPDATE navstevnost stat SET stat.count = '$checkStatistic' WHERE stat.id = 3";
}

if(isset($_POST['submit'])){
	$r = $_POST['r'];
	$sql = "UPDATE vstup SET gulicka='$r' WHERE id=1";
	$conn->query($sql);
}

/*
$sql2 = "SELECT gulicka FROM vstup WHERE id=1";
  ...
  margin-bottom: 100pxô
	...
	<body id="top" data-spy="scroll" data-target=".navbar-collapse" data-offset="50">
     ...
     <div class="check">
		...
		<form method="post" action="gulicka.php">
		<br><p>Zadaj nove r:</p><input type="number" step="1" id="r" name="r" min="0" max="200">
	...
	var stream = null;
            ...
            var url = "http://147.175.121.210:8168/skuska/server2.php";
	...
	<script>
			...
			function draw() {
				...
				pozicia = parseFloat(document.getElementById("result2").innerHTML);
							
				if(target[12] === "-"){
					document.getElementById("result3").innerHTML = parseFloat(target.substring(30, 41));
					uhol = parseFloat(document.getElementById("result3").innerHTML);
				}else if(ci > 99){
					document.getElementById("result3").innerHTML = parseFloat(target.substring(31, 42));
					uhol = parseFloat(document.getElementById("result3").innerHTML);
				}else if(ci > 9){
					document.getElementById("result3").innerHTML = parseFloat(target.substring(30, 41));
					uhol = parseFloat(document.getElementById("result3").innerHTML);
				}else{ //neni minus
					document.getElementById("result3").innerHTML = parseFloat(target.substring(29, 40));
					uhol = parseFloat(document.getElementById("result3").innerHTML);
				}
